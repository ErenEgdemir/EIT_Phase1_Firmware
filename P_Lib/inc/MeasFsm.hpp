...
public:
	...
	void applyCfg(eit_cfg_t cfg);
	void applyMapTable(eit_map_t* m);
    void requestStart();
	void requestStop();

	using DoneCb = void(*)(void* user, const float* Ipp, const float* Qpp, uint32_t count);
	...
	void handleSettlingIRQ();
	void handleIntegrateTimer_IRQ(uint32_t ndtr, uint32_t t);
	void handleIntegrateADC_IRQ(void);
	void freqIndentifier(void);
