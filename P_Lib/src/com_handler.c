...
static uint8_t g_tx_active_buf[4 + MAX_PAYLOAD + 2];
static uint16_t g_tx_active_len = 0;
static uint8_t g_tx_pending_buf[4 + MAX_PAYLOAD + 2];
static uint16_t g_tx_pending_len = 0;
...
static inline float rd_f32le(const uint8_t* p)
{
    uint32_t raw = rd_u32le(p);
    float v;
...
static bool usb_try_send(uint8_t* data, uint16_t len)
{
	if(CDC_Transmit_FS(data, len) == USBD_OK) return true;
	return false;
}
...
static uint16_t build_frame_bytes(uint8_t* dst, const uint8_t* payload, uint16_t len)
{
	dst[0] = 0xA5;
	dst[1] = 0x5A;
	dst[2] = (uint8_t)len & 0xFF;
	dst[3] = (uint8_t)((len >> 8) & 0xFF);
	if(len && payload) memcpy(&dst[4], payload, len);

	uint16_t crc = crc16_ccitt(payload, len);
	dst[4 + len] = (uint8_t)(crc & 0xFF);
	dst[5 + len] = (uint8_t)((crc >> 8) & 0xFF);
	return (uint16_t)(4 + len + 2);
}
...
static void send_frame(const uint8_t* payload, uint16_t len)
{
	...
	if(usb_tx_busy()) {
		g_tx_pending_len = build_frame_bytes(g_tx_pending_buf, payload, len);
		g_tx_pending = true;
		return;
	}

	g_tx_active_len = build_frame_bytes(g_tx_active_buf, payload, len);
	if(!usb_try_send(g_tx_active_buf, g_tx_active_len)) {
		memcpy(g_tx_pending_buf, g_tx_active_buf, g_tx_active_len);
		g_tx_pending_len = g_tx_active_len;
		g_tx_pending = true;
	}
}
...
{
	static uint8_t pl[MAX_PAYLOAD];
	static uint8_t debug[29];
	wr_u32le(&debug[0], d.phase);
	wr_u32le(&debug[4], d.phaseInc);
	wr_u32le(&debug[8], (uint32_t)d.phaseCorr);
	wr_f32le(&debug[12], d.freqError);
	wr_f32le(&debug[16], d.freqi);
	wr_u32le(&debug[20], d.phaseError);
	debug[21] = d.st;
	wr_u32le(&debug[22], d.measuredFreq);
...
static void handle_payload(const uint8_t* pl, uint16_t len)
{
	...
	case CMD_SET_PARAM: {
		...
		g_cfg.freqKpFine 			= rd_f32le(&args[14]);
		g_cfg.freqKiFine 			= rd_f32le(&args[18]);
		g_cfg.phaseKpFine 			= rd_f32le(&args[22]);
		g_cfg.freqKpCoarse 			= rd_f32le(&args[26]);
		g_cfg.freqKiCoarse 		    = rd_f32le(&args[30]);
		g_cfg.phaseKpCoarse		    = rd_f32le(&args[34]);

		g_cfg.PhaseIdeal			= rd_u32le(&args[38]);
		g_cfg.FreqErrorMaxFine 		= rd_f32le(&args[42]);
		g_cfg.PhaseErrorMaxFine 	= rd_u32le(&args[46]);

		g_cfg.FreqErrorMaxCoarse 	= rd_f32le(&args[50]);
		g_cfg.FreqErrorMinCoarse 	= rd_f32le(&args[54]);
		g_cfg.PhaseErrorMaxCoarse 	= rd_u32le(&args[58]);
	...
	case CMD_SET_MAP:
		...
		for(int i = 0; i < 208; i++) {
			g_map[i].i_p = args[4*i];
			
			g_map[i].i_n = args[4*i+1];
			
			g_map[i].v_p = args[4*i+2];
			
			g_map[i].v_n = args[4*i+3];
